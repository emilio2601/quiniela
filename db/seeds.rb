...
#
# Idempotent: a fixture is keyed on its 1..104 match number, so re-running
# refreshes kickoff times without disturbing recorded scores or picks. Team
# names are only set when a fixture is first created — knockout placeholders
# (W97, 3CDFG, …) get resolved to real teams by the results poller, and a
# re-seed must never stomp on that.
require "json"
require "time"
...
fixtures.each_with_index do |m, index|
  match = Match.find_or_initialize_by(number: index + 1)
  match.kickoff_at = parse_kickoff(m["date"], m["time"])

  if match.new_record?
    match.home_team = m["team1"]
    match.away_team = m["team2"]
    created += 1
  elsif match.changed?
    updated += 1
  end
